...
class gapiOAuth2 {
  ...
  public function fetchToken($client_email, $key_file, $delegate_email = null) {
    $header = array(
      "alg" => static::header_alg,
      "typ" => static::header_typ,
    );

    $claimset = array(
      "iss" => $client_email,
      "scope" => static::scope_url,
      "aud" => static::request_url,
      "exp" => time() + (60 * 60),
      "iat" => time(),
    );
    ...
    $url = new gapiRequest(static::request_url);
...
class gapiRequest {
  ...
  public function request($get_variables=null, $post_variables=null, $headers=null) {
    $interface = static::http_interface;

    if (static::http_interface == 'auto')
    ...
    switch ($interface) {
      ...
      default:
        throw new Exception('Invalid http interface defined. No such interface "' . static::http_interface . '"');
    }
  }
